...
namespace WebAPI.Middleware
{
    public class ManejadorErrorMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            try
            {
                //Si el contexto esta bien (es decir los parametros que envia el cliente) entonces el conexto 
                //pasa a la siguiente capa,sino lo atrapa el manejador de excepcion
                await _next(context); 
            }
            catch (Exception ex)
            {
